...
- 控制台会完整输出 EMClient SDK 调用、响应、事件、失败与重连日志，并用前缀区分 SDK 原始日志、App 事件派发、Store/UI 外层处理日志，便于排查 socket、消息和聊天室链路。
- IM 连接断开、发送超时等真实失败仅打印到控制台，不使用开发态全屏错误覆盖层遮挡页面。
- socketUrl 显式配置 `ws://` 或 `wss://` 时原样使用，未配置协议时默认补 `wss://`。
- 消息 `chatType` 以 SDK / 服务端真实返回为准，缺失时清晰报错，不本地推断为单聊、群聊或聊天室；跨端编辑/撤回事件缺少 `chatType` 时，仅可使用本地已存在原消息的真实 `chatType` 更新。
- 登录、NGI/TKE/DEV/QA隔舱服务配置、主导航、会话管理、单个会话推送通知方式与免打扰时长、联系人详情与好友关系维护。
- 用户资料、在线状态、个人设置、SDK 日志与退出登录。
- 单聊/群聊消息收发、透传消息发送与接收展示、消息引用、复制、撤回、编辑、删除、举报、置顶、已读送达状态。
- 群组详情、群头像、群扩展信息、群公告、群共享文件、群成员、黑名单、禁言、免打扰、退群与解散群组；群组系统通知基于 SDK 真实事件字段展示成员/操作者，缺失时不显示 undefined。
- 单聊/群聊 Reaction。
- 聊天室列表、加入退出、详情刷新、公告、自定义属性、成员管理；加入请求进行中会阻止同一聊天室重复提交，结果仍以真实 SDK / 服务端返回为准。
- 聊天室消息全链路能力。
